...
BACKUP_FILE = "backup_timers.json"
...
def guardar_backup_local(data):
    """Guarda un respaldo de los datos actuales en el disco."""
    try:
        with open(BACKUP_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"Error escribiendo backup local: {e}")

def cargar_backup_local():
    """Carga los datos respaldados en caso de falla con Supabase."""
...
def obtener_datos():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            raise Exception("Respuesta no válida de Supabase")
    except Exception as e:
        print(f"Falla de conexion a Supabase ({e}). Utilizando datos del backup local.")
        backup = cargar_backup_local()
        if backup:
            return backup.get("timers", timers_map), backup.get("pcs", pcs_map), backup.get("pjs", pj_map), backup.get("hb", hb_map)
        return timers_map, pcs_map, pj_map, hb_map
...
<head>
    ...
    <style>
        ...
        /* PANEL DE REGISTRO MANUAL SUPERIOR */
        .manual-panel {
        ...
        .timer-badge { font-family: monospace; font-size: 0.85rem; font-weight: bold; padding: 4px 6px; border-radius: 6px; text-align: center; width: 100%; box-sizing: border-border-box; margin-bottom: 8px; }
...
<body>
    ...
    <!-- PANEL REGISTRO MANUAL SUPERIOR -->
        ...
        function render() {
            ...
            servidoresAMostrar.forEach(svr => {
                ...
                for (const [bossName, cdMinutos] of Object.entries(cooldowns)) {
                    ...
                    let displayTimer = '';
                    let prioridadOrden = 0; // Para ordenar de mayor a menor urgencia
                    ...
                    if (bossName in bossesServidor) {
                        ...
                        if (["Yellow Goblin", "Blue Goblin", "Red Goblin"].includes(bossName)) {
                            ...
                            if (ahoraUnix < inicioVentanaUnix) {
                                ...
                                statusState = 'window';
                                prioridadOrden = -500; // Alta prioridad en ventana
                                ...
                                // Pasó la ventana -> Vivo (tiempo transcurrido desde finVentana)
                            ...
                            // cd terminado -> Vivo (tiempo transcurrido ascendente)
                        ...
                        // Si no hay registro previo, está vivo de forma indeterminada
                        prioridadOrden = -999;
                ...
                // ORDENAR DE MENOR TIEMPO A MAYOR TIEMPO DE ESPERA (Priorizando los que van a salir o ya salieron)
...
# ACCIONES DE KILL/RESET ADAPTADAS A GET Y POST (Soporta custom_timer manual)
